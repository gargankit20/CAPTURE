...
class RegisterViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingImage image: UIImage, editingInfo: [String : AnyObject]?) {
        let imageEditorViewController = TOCropViewController(croppingStyle: TOCropViewCroppingStyle.default, image: image);
        if changeBG {
            imageEditorViewController.aspectRatioPreset = .preset16x9
            let scale = UIScreen.main.scale
            imageEditorViewController.imageCropFrame = CGRect(x:0, y:0, width:image.size.width*scale, height:image.size.width*scale*(9.0/16.0))
        } else {
            imageEditorViewController.aspectRatioPreset = .presetSquare
        }
        imageEditorViewController.delegate = self
        picker.pushViewController(imageEditorViewController, animated: true)
    }
